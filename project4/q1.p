...
is_pq(X, _, [PQH | _]) :- course(PQH, PQ, _), is_pq(X, PQ, PQ). 
is_pq(X, _, [_ | PQTail]) :- is_pq(X, PQTail, PQTail).

allprereq(Prereqs, Course) :- findall(X, (course(X, _, _), course(Course, PQ, _), is_pq(X, PQ, PQ)), Prereqs), sort(Prereqs).

/* (f)
student_teach: from a course, get all the students, and find all the teachers each student is currently taking (input: teachers and a course)
get_teachers: checks each students current courses to each teachers course list (recursive)
*/

get_teachers(CourseList, [CH | _]) :- member(CH, CourseList).
get_teachers(CourseList, [_ | CTail]) :- get_teachers(CourseList, CTail).

student_teach(Teachers, Course) :- findall(X, (student(_ , CurrC, _), member(Course, CurrC), instructor(X, CourseList), get_teachers(CourseList, CurrC)), Teachers), sort(Teachers).